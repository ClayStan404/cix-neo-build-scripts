#!/usr/bin/env bash
# Shared sbuild implementation for externally packaged DKMS source projects.
set -Eeuo pipefail
...
: "${CIX_DKMS_TARGET:?CIX_DKMS_TARGET is required}"
: "${CIX_DKMS_SOURCE_REL:?CIX_DKMS_SOURCE_REL is required}"
: "${CIX_DKMS_GIT_REL:?CIX_DKMS_GIT_REL is required}"
: "${CIX_DKMS_PACKAGING_REL:?CIX_DKMS_PACKAGING_REL is required}"
: "${CIX_DKMS_LABEL:?CIX_DKMS_LABEL is required}"

action="build"
distribution="${CIX_BUILD_DISTRIBUTION:-trixie}"
jobs="${CIX_BUILD_JOBS:-$(cix_default_jobs)}"
nexus_site="${CIX_NEXUS_SITE:-zj}"
output_dir="${CIX_OUTPUT_DIR:-${CIX_WORKSPACE_ROOT}/output}/${CIX_DKMS_TARGET}"

usage() {
    cat <<EOF
Usage: $(basename "$0") [OPTIONS] [build|clean]

Assemble the ${CIX_DKMS_LABEL} source and external Debian metadata, create a
source package, and build it with the native ARM64 sbuild environment.
...
[[ "${CIX_DKMS_TARGET}" =~ ^[a-z0-9][a-z0-9-]*$ ]] ||
    cix_die "invalid DKMS target: ${CIX_DKMS_TARGET}"

readonly source_dir="${CIX_WORKSPACE_ROOT}/${CIX_DKMS_SOURCE_REL}"
readonly source_git="${CIX_WORKSPACE_ROOT}/${CIX_DKMS_GIT_REL}"
readonly packaging_dir="${CIX_WORKSPACE_ROOT}/${CIX_DKMS_PACKAGING_REL}"
...
[[ -f "${packaging_dir}/control" ]] ||
    cix_die "${CIX_DKMS_LABEL} Debian metadata is missing: ${packaging_dir}"
[[ -f "${sbuild_config}" ]] || cix_die "sbuild configuration is missing: ${sbuild_config}"

if [[ "${action}" == "clean" ]]; then
    if [[ -d "${artifacts_dir}" ]]; then
        cix_log "Remove ${CIX_DKMS_LABEL} package artifacts from ${artifacts_dir}"
        find "${artifacts_dir}" -mindepth 1 -maxdepth 1 -type f -delete
    fi
    exit 0
fi

cix_require_command dpkg-parsechangelog dpkg-source find git realpath rsync sbuild tar
[[ -d "${source_dir}" && -f "${source_dir}/dkms.conf" ]] ||
    cix_die "${CIX_DKMS_LABEL} DKMS source is missing: ${source_dir}"
git -C "${source_git}" rev-parse --is-inside-work-tree >/dev/null 2>&1 ||
    cix_die "${CIX_DKMS_LABEL} source is not a Git worktree: ${source_git}"
[[ -s "${sbuild_chroot}" ]] ||
...
work_root="$(mktemp -d "${output_dir}/.${CIX_DKMS_TARGET}.XXXXXXXXXX")"
readonly work_root
...
cix_log "Assemble ${CIX_DKMS_LABEL} source package in ${work_root}"
mkdir -p -- "${source_tree}"
...
cix_log "${CIX_DKMS_LABEL} DKMS package build complete"
