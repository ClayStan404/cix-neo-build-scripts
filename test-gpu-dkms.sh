#!/usr/bin/env bash
# Build the packaged GPU DKMS modules against packaged CIX kernel headers.
set -Eeuo pipefail
...
kernel_headers_deb=""
gpu_dkms_deb=""
output_dir="${CIX_OUTPUT_DIR:-${CIX_WORKSPACE_ROOT}/output}/gpu-dkms/tests"

usage() {
    cat <<'EOF'
Usage: test-gpu-dkms.sh [OPTIONS]

Build the modules shipped by cix-gpu-dkms against the headers shipped by the
CIX kernel build. All DKMS source, state, and module trees are temporary; the
host /usr/src, /var/lib/dkms, and /lib/modules trees are not modified.

Options:
  --kernel-headers PATH  CIX linux-headers deb; auto-detected if unambiguous
  --gpu-package PATH     cix-gpu-dkms deb; auto-detected if unambiguous
...
if [[ -z "${gpu_dkms_deb}" ]]; then
    gpu_dkms_deb="$(select_single_artifact \
        "${CIX_WORKSPACE_ROOT}/output/gpu-dkms/artifacts" \
        'cix-gpu-dkms_*_all.deb' 'GPU DKMS package')"
fi

kernel_headers_deb="$(realpath -e -- "${kernel_headers_deb}")"
gpu_dkms_deb="$(realpath -e -- "${gpu_dkms_deb}")"
output_dir="$(realpath -m -- "${output_dir}")"
readonly kernel_headers_deb gpu_dkms_deb output_dir

header_package="$(dpkg-deb -f "${kernel_headers_deb}" Package)"
header_architecture="$(dpkg-deb -f "${kernel_headers_deb}" Architecture)"
gpu_package="$(dpkg-deb -f "${gpu_dkms_deb}" Package)"
gpu_architecture="$(dpkg-deb -f "${gpu_dkms_deb}" Architecture)"
...
[[ "${gpu_package}" == "cix-gpu-dkms" ]] ||
    cix_die "unexpected GPU package: ${gpu_package}"
[[ "${gpu_architecture}" == "all" ]] ||
    cix_die "GPU DKMS package must be architecture all: ${gpu_architecture}"
...
mkdir -p -- "${output_dir}"
work_root="$(mktemp -d "${output_dir}/.gpu-dkms-test.XXXXXXXXXX")"
...
cleanup() {
    ...
    trap - EXIT
    if [[ -n "${build_log_source}" && -f "${build_log_source}" && \
        -n "${result_log}" ]]; then
...
cix_log "Extract GPU DKMS package: ${gpu_dkms_deb}"
dpkg-deb -x "${gpu_dkms_deb}" "${package_root}"
...
module_name="$(awk -F '"' '/^PACKAGE_NAME="[^"]+"$/ { print $2; exit }' \
    "${module_config}")"
module_version="$(awk -F '"' '/^PACKAGE_VERSION="[^"]+"$/ { print $2; exit }' \
    "${module_config}")"
...
cix_log "Build GPU modules for CIX kernel ${kernel_release}"
"${dkms_command}" build \
    -m "${module_name}" \
    -v "${module_version}" \
    -k "${kernel_release}" \
...
cix_log "GPU DKMS compatibility test passed; build log: ${result_log}"
