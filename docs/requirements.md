...
- Provide `clean-all` for target-owned cleanup of every target registered in
  `build-map.yaml`; remove empty output directory trees, including retired
  target leftovers, while retaining directories that contain reusable caches
  or unregistered artifacts. Provide
  ...
  sbuild chroot and nonempty unregistered output entries; report every
  preserved output entry instead of deleting data outside the registry.
...
- Publish the complete Sky1 product signing matrix generated by the pinned
  internal package flow: PR release, PR debug, PR2 debug, prototype release,
  and prototype debug, each in Full and OTA layouts. Keep board names in every
  published filename and collect only Full layouts in the OCB directory.
...
emulation, or substitute the older Sky1 CBFF format. The authoritative source
is maintained in restricted repository `cix_security/tool`; a target can be
